...
def make_publication_plot3(ax, df, simulation, system_name,
    ...
    if system_name == "All":
        ...
        stats_text = (
            ...
            # fr"  \mathrm{{ocMSE}} &= {mse[0]}_{{{mse[1]}}}^{{{mse[2]}}} \\"
    ...
    ax.text(
        stats_loc[0], stats_loc[1], stats_text, transform=ax.transAxes, fontsize=31 if main_fig else 12,
        verticalalignment=verticalalignment, horizontalalignment=horizontalalignment, bbox=bbox
        )
    ...
    if return_stats:
        return scatter, (pearson, kendall, mse, rmse) # mue, rmse)
    else:
        return scatter
    ...
    for CALC_TYPE in ["mbar", "dh-gb"]:
        for FILTER_THROMBIN_LIG_4 in [True, False]:
            ...
            for SELECTED_FORCE_FIELD in FORCE_FIELD_ORDER:
                ...
                df_merge, df_merge_mse_removed = get_all_data(
                    CALC_TYPE=CALC_TYPE,
                ...
                padding = 1
                min_value_mse_removed = np.min(df_merge_mse_removed[[f"simulation_{SELECTED_FORCE_FIELD}", "exp_dG"]]) - padding
                max_value_mse_removed = np.max(df_merge_mse_removed[[f"simulation_{SELECTED_FORCE_FIELD}", "exp_dG"]]) + padding

                min_value = np.min(df_merge[[f"simulation_{SELECTED_FORCE_FIELD}", "exp_dG"]]) - padding
                max_value = np.max(df_merge[[f"simulation_{SELECTED_FORCE_FIELD}", "exp_dG"]]) + padding
                ...
                for ax, system in zip(axis_individual, local_system_order):
                    ...
                    _ = make_publication_plot3(
                        ax, df=df_merge[df_merge["source"] == system], simulation=f"simulation_{SELECTED_FORCE_FIELD}",
                ...
                scatter, stats = make_publication_plot3(
                    ...
                    min_value=min_value_mse_removed,
                    max_value=max_value_mse_removed,
                    return_stats=True,
                    calc_type=CALC_TYPE,
                    )
                ...
                if FILTER_THROMBIN_LIG_4:
                    fig.savefig(f"summary/ff-v2/{CALC_TYPE}_{SELECTED_FORCE_FIELD}_thrombin_lig_4_removed.pdf",
                                bbox_inches="tight",
                                pad_inches=0.05,
                                transparent=False)
                else:
                    fig.savefig(f"summary/ff-v2/{CALC_TYPE}_{SELECTED_FORCE_FIELD}.pdf",
                                bbox_inches="tight",
                                pad_inches=0.05,
                                transparent=False)
